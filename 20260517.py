...
# ─────────────────────────────────────────────────────────────────────────────
# nincs használva még
def average_checkpoints(checkpoint_dirs: list[str], tokenizer) -> None:
    """
    Load the top-k checkpoints, average their weights, and save the result
    to CFG.save_dir as the final production model.
    """
    print(f"\n[Checkpoint Averaging] Averaging {len(checkpoint_dirs)} checkpoints...")
    models = [
        AutoModelForSeq2SeqLM.from_pretrained(d).state_dict()
        for d in checkpoint_dirs
    ]
    avg_state = {
        k: torch.stack([m[k].float() for m in models], dim=0).mean(dim=0)
    ...
    final_model.save_pretrained(CFG.save_dir)
    tokenizer.save_pretrained(CFG.save_dir)
    print(f"  Averaged model saved to: {CFG.save_dir}")
...
def train(cfg: Config) -> None:
    ...
    model_val     = compute_all_metrics(val_preds,  val_refs, "Validation — Model")
